...
function animateValue(element, start, end, duration, suffix = "") {
  ...
  function update(currentTime) {
    ...
    if (suffix === "%") {
      element.textContent =
        (start + (end - start) * easedProgress).toFixed(1) + suffix;
    } else if (suffix === "K+") {
...
function resetAnimationClasses(element) {
  const animationClasses = [
    ...
    "phone-animate",
    "action-card-animate",
    "assistant-demo-animate",
    "platform-item-animate",
    "animate",
  ];
...
function initClientsSection() {
  ...
  if (clientItems.length > 0 && clientsSection) {
    console.log(
      "Initializing clients section with",
      clientItems.length,
      "items",
    );
...
function initWhatsAppWidget() {
  ...
  function ensureHighZIndex() {
    ...
    if (highestZIndex > 10000) {
      document.querySelector(".whatsapp-widget").style.zIndex =
        highestZIndex + 1;
    }
  }
...
document.getElementById("contactForm").addEventListener("submit", function (e) {
  ...
  // Send to Formspree
  fetch("https://formspree.io/f/xeelbewz", {
    method: "POST",
    body: formData,
    headers: {
